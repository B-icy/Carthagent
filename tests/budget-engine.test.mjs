...
test('actual engine stops tool loop, restores budget and accepts explicit reset offline', { timeout: 45000 }, async t => {
  ...
  async function run(prompt) {
    const child = spawn(process.execPath, [cli, '-p', '--no-guide', '--provider', 'localtest', '--model', 'fixture', '--session', session, '--max-tools', '1', '--max-seconds', '10', prompt], { cwd, env: { ...process.env, PI_OFFLINE: '1', PI2_CODING_AGENT_DIR: agent, PI_CODING_AGENT_DIR: agent } });
    child.stdin.end();
  ...
  assert.equal(toolResults(entries).length, 1);
  assert.equal(entries.filter(e => e.customType === 'delivery-budget-v1').at(-1).data.stopped, 'tool-calls');
  assert.ok(calls <= 2, `unexpected followups: ${calls}`);
  ...
  entries = await run('Read again');
  assert.equal(toolResults(entries).length, 2);
});
