...
4. Stop after at most 3 review rounds — if findings remain, summarize them for the user instead of looping forever.`;
}
...
/** Extract the reviewer's verdict line from its output tail. */
export function parseVerdict(output) {
  const match = /VERDICT:\s*(APPROVE|CHANGES-REQUESTED)/i.exec(String(output ?? ''));
