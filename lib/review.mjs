 ...
 * and opens a PR; `carthagent review <pr>` then runs a brand-new agent process with
 ...
 * explicit flag (--review / --delivery-review) > ~/.carthagent/config.json > 'ask'.
 */
...
/** Persistent local PR review admission limit; not a security boundary. */
...
/** carthagent's user-level config file (CARTHAGENT_CONFIG overrides for tests). */
export function carthagentConfigPath() {
  return process.env.CARTHAGENT_CONFIG || join(os.homedir(), '.carthagent', 'config.json');
}

export function loadCarthagentConfig(path = carthagentConfigPath()) {
  try {
...
/** Merge a patch into ~/.carthagent/config.json atomically. Returns the new config. */
export function saveCarthagentConfig(patch, path = carthagentConfigPath()) {
  const config = { ...loadCarthagentConfig(path), ...patch };
  atomicJson(path, config);
  return config;
}

/** flag (explicit CLI/extension flag) > config file > 'ask'. */
export function resolveReviewMode(flagValue, config = loadCarthagentConfig()) {
  return normalizeReviewMode(flagValue) ?? normalizeReviewMode(config?.review) ?? 'ask';
}

/** Resolve sticky theme/model defaults: explicit flag > config > undefined.
 *  Undefined falls through to the built-in default (opencode theme, agent's
 *  default model) so we don't pass a stale `--model`/`--theme` to the engine. */
export function resolveStickyDefaults(opts, config = loadCarthagentConfig()) {
  return {
    theme: opts?.theme || config?.theme || undefined,
    model: opts?.model || config?.model || undefined,
  };
}
...
const CARTHAGENT_BIN = join(ROOT, 'bin', 'carthagent.mjs');
 ...
 */
export function reviewKickoff(carthagentBin = CARTHAGENT_BIN) {
...
2. Run: node "${carthagentBin}" review <pr-number-or-url>
...
4. Stop after at most ${MAX_REVIEW_ROUNDS} review rounds — the CLI persists admissions per PR in this workspace, including failed attempts. At the limit, summarize remaining findings and escalate to the user instead of looping forever.`;
}

/** Prompt for the detached fresh-context reviewer subprocess (`carthagent review <pr>`). */
...
export function parseVerdict(output) {
  const last = String(output ?? '').trim().split(/\r?\n/).at(-1);
  const match = /^VERDICT:\s*(APPROVE|CHANGES-REQUESTED)(?:\s+—\s+[^\r\n]+)?$/i.exec(last);
