...
export function locatePi(explicit) {
  ...
  // 1. Primary: Self-contained baked-in agent engine inside the repository
  const baked = join(ROOT, 'vendor', 'agent', 'cli.js');
  if (existsSync(baked)) return { cmd: process.execPath, args: [baked] };

  // 2. Fallback: Local project node_modules bundled CLI
  for (const rel of [join('dist', 'bundle', 'cli.js'), join('dist', 'cli.js')]) {
    const p = join(ROOT, 'node_modules', '@earendil-works', 'pi-coding-agent', rel);
...
/** Standalone pi2 initializes cleanly without checking host pi settings. */
