...
// Load jsdom only for DOM checks; static serving and WebDriver need no DOM runtime.
...
export function firefoxScreenshot(url, out, { width = 1280, height = 800, timeoutMs = 45000, waitSeconds } = {}) {
  return new Promise((ok) => {
    ...
    const profile = mkdtempSync(join(tmpdir(), 'carthagent-firefox-'));
 ...
 *
 * Module scripts are unsupported by jsdom and fail execution-enabled checks.
 * Use runScripts:false only for static DOM inspection, not application verification.
...
export async function domCheck(source, assertions = [], { url, runScripts = true, waitMs = 250 } = {}) {
  ...
  const failures = [];
  if (runScripts && [...document.querySelectorAll('script')].some(script =>
    (script.getAttribute('type') || '').toLowerCase() === 'module')) {
    failures.push('Unsupported module scripts: jsdom does not execute them. Use a real-browser check for application verification, or runScripts:false for static DOM inspection only.');
  }
  const run = a => {
    try {
      if (a.eval) {
