 ...
 * (bin/ctg.mjs, lib/tui/app.mjs) and the evaluation runner (evaluate.mjs).
 * carthagent uses a baked-in agent engine in vendor/agent and an isolated state directory
 * (~/.carthagent/agent) to ensure it runs completely fresh without piggybacking on any
...
// Ensure an isolated state directory (~/.carthagent/agent) by default:
const DEFAULT_AGENT_DIR = process.env.CARTHAGENT_AGENT_DIR || process.env.CARTHAGENT_CODING_AGENT_DIR || join(os.homedir(), '.carthagent', 'agent');
if (!process.env.CARTHAGENT_CODING_AGENT_DIR && !process.env.CARTHAGENT_AGENT_DIR) {
  process.env.CARTHAGENT_CODING_AGENT_DIR = DEFAULT_AGENT_DIR;
}
if (!process.env.PI_CODING_AGENT_DIR) {
  process.env.PI_CODING_AGENT_DIR = process.env.CARTHAGENT_CODING_AGENT_DIR || DEFAULT_AGENT_DIR;
}

export function locateEngine(explicit) {
  const candidates = [explicit, process.env.CARTHAGENT_CLI, process.env.PI_CLI].filter(Boolean);
...
/** Build engine CLI args for both the TUI (rpc) and headless (-p) paths. */
export function buildEngineArgs(opts) {
  ...
  // Product providers load independently of the delivery extension so they are
  // available in the TUI, RPC child, headless runs, and --no-delivery mode.
  args.push('-e', join(ROOT, 'extensions', 'experiential.ts'));
...
/** carthagent's per-cwd session dir: <agentDir>/sessions/--a-b-c--/. Defaults to ~/.carthagent/agent. */
export function sessionDirFor(cwd = process.cwd(), agentDir = process.env.CARTHAGENT_AGENT_DIR || process.env.CARTHAGENT_CODING_AGENT_DIR || process.env.PI_CODING_AGENT_DIR || join(os.homedir(), '.carthagent', 'agent')) {
