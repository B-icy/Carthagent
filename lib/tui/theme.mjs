/** carthagent theme palettes — opencode token set, Node-only, zero dependencies.
 *
 ...
 * resolve against `defs`. Legacy carthagent keys (accent, muted, faint, ok, warn,
...
export function flattenTheme(raw, mode = DEFAULT_MODE) {
  ...
  // Legacy carthagent aliases so existing render code keeps working.
...
const RAW_THEMES = {
  // Carthagent's default: Tyrian purple surfaces with high-contrast Punic gold.
  carthage: {
    label: 'Carthage',
    defs: {},
    theme: {
      primary: '#f2c66d', secondary: '#c7a5ff', accent: '#f2c66d',
      error: '#ff969e', warning: '#f2c66d', success: '#76d6aa', info: '#c7a5ff',
      text: '#f7f1e5', textMuted: '#b8a9c9',
      background: '#120d1c', backgroundPanel: '#1d1429', backgroundElement: '#2a1d3c',
      border: '#49385d', borderActive: '#f2c66d', borderSubtle: '#372947',
      diffAdded: '#76d6aa', diffRemoved: '#ff969e',
      diffContext: '#b8a9c9', diffHunkHeader: '#c7a5ff',
      diffHighlightAdded: '#76d6aa', diffHighlightRemoved: '#ff969e',
      diffAddedBg: '#173328', diffRemovedBg: '#3a1c2a', diffContextBg: '#1d1429',
      diffLineNumber: '#b8a9c9',
      diffAddedLineNumberBg: '#173328', diffRemovedLineNumberBg: '#3a1c2a',
      markdownText: '#f7f1e5', markdownHeading: '#f2c66d',
      markdownLink: '#c7a5ff', markdownLinkText: '#ead9ff', markdownCode: '#76d6aa',
      markdownBlockQuote: '#b8a9c9', markdownEmph: '#f2c66d', markdownStrong: '#ead9ff',
      markdownHorizontalRule: '#8b76a3',
      markdownListItem: '#f2c66d', markdownListEnumeration: '#c7a5ff',
      markdownImage: '#c7a5ff', markdownImageText: '#ead9ff', markdownCodeBlock: '#f7f1e5',
      syntaxComment: '#a99abb', syntaxKeyword: '#c7a5ff', syntaxFunction: '#f2c66d',
      syntaxVariable: '#f7f1e5', syntaxString: '#76d6aa', syntaxNumber: '#f2c66d',
      syntaxType: '#ead9ff', syntaxOperator: '#c7a5ff', syntaxPunctuation: '#f7f1e5',
    },
  },
  ...
  // Legacy carthagent palettes, expressed in the opencode token set.
  nebula: {
    label: 'Nebula',
    defs: {},
    theme: {
 ...
 * CARTHAGENT_THEME_MODE=light|dark overrides; otherwise COLORFGBG's background
 * index (0-6 or 8 = dark ANSI colors, 7 or 9-15 = light); default dark. */
export function detectTermMode(env = {}) {
  const forced = String(env.CARTHAGENT_THEME_MODE || '').toLowerCase();
...
export function getTheme(name, mode = DEFAULT_MODE) {
  ...
  return palettes[key] || palettes.carthage;
}

/** Resolve an env/flag theme name, defaulting to Carthagent's own palette. */
export function resolveThemeName(name) {
  if (!name) return 'carthage';
  const n = String(name).toLowerCase();
  return THEME_NAMES.find(k => k === n || (RAW_THEMES[k].label || '').toLowerCase() === n) || 'carthage';
}
