...
/** Quick lightweight check: does the user have any provider credentials at all? */
export function hasConfiguredProvider(env = process.env, agentDir = AGENT_DIR) {
  if (existsSync(join(agentDir, 'auth.json'))) return true;
  if (existsSync(join(agentDir, 'models.json'))) return true;
...
export function loadAuthRuntime() {
  if (!runtimePromise) {
    runtimePromise = import('../../vendor/agent/index.js')
      .then(async engine => registerExperientialProvider(await engine.ModelRuntime.create({})))
