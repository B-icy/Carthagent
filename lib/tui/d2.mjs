 ...
 */
import { fg, bg, mix, width, wrap } from './ansi.mjs';
...
export function renderD2(graph, opts) {
  const { width: W, theme: T, states = new Map(), packet = null, flash = new Set(), frame = 0, phaseIndex = -1 } = opts;
  ...
  // Node interior fills: diff bg tones blended toward the panel so boxes
  // read as tinted surfaces instead of bare outlines.
  const fillFor = st => {
  ...
  const topJoin = id => milestone(id) ? '╤' : '┬';
  const botJoin = id => milestone(id) ? '╧' : '┬';
  ...
  for (const n of graph.nodes) {
    ...
    if (st === 'active') { const p = (Math.sin(frame / 8) + 1) / 2; border = mix(accent, hot, p * 0.45); mods = BOLDM; label = T.text; }
    ...
    if (sameCol) {
      ...
      if (x === tx) {
        ...
        for (let y = a.bottom + 1; y < b.top - 1; y++) {
          ...
          put(x, y, pk ? (frame % 2 ? '●' : '◦') : vch, pk ? hot : c);
        }
        // Arrowhead into the active node breathes: hollow ~1/4 cycle, color
        // pulsed toward hot on the active-border phase.
        const toActive = (states.get(e.to) || 'pending') === 'active';
        const pulse = toActive ? (Math.sin(frame / 8) + 1) / 2 : 0;
        put(tx, b.top - 1, toActive && frame % 8 < 2 ? '▽' : '▼',
          hotEdge ? hot : toActive ? mix(c, hot, 0.3 + pulse * 0.45) : c);
        put(tx, b.top, topJoin(e.to), states.get(e.to) === 'pending' ? T.border : c);
      } else {
        for (let y = a.bottom + 1; y < b.top - 1; y++) put(x, y, vch, c);
        put(x, b.top - 1, x < tx ? '╰' : '╯', c);
        hline(Math.min(x, tx) + 1, Math.max(x, tx) - 1, b.top - 1, hch, c);
        put(tx, b.top - 1, x < tx ? '╮' : '╭', c);
      ...
      hline(a.right + 1, chX - 1, aMid, '─', c);
      put(chX, aMid, '╯', c);
      for (let y = bMid + 1; y < aMid; y++) put(chX, y, '│', c);
      put(chX, bMid, '╮', c);
      hline(b.right + 1, chX - 1, bMid, '─', c);
      // Same blink on the side-channel arrowhead when it lands on the active
      // node — color stays (back-edges are the failure channel).
      put(b.right, bMid, (states.get(e.to) || 'pending') === 'active' && frame % 8 < 2 ? '◁' : '◀', c);
    } else {
      ...
      put(x, a.bottom, botJoin(e.from), c);
      for (let y = a.bottom + 1; y < b.top - 1; y++) put(x, y, '│', c);
      const tx = centerX.get(e.to);
      put(x, b.top - 1, '╰', c);
      hline(x + 1, tx - 1, b.top - 1, '─', c);
      put(tx, b.top - 1, '╮', c);
      put(tx, b.top, topJoin(e.to), c);
      if (e.label) {
        const lx = x + 2;
