...
export function renderD2(graph, opts) {
  ...
  const { rank, col, back } = layoutD2(graph);
  ...
  for (const n of graph.nodes) {
    ...
    const maxBw = Math.max(4, W - 2);
    boxW.set(n.id, Math.min(maxBw, Math.max(Math.min(9, maxBw), avail)));
  }
  ...
  for (const n of graph.nodes) {
    ...
    const left = Math.max(0, c === 1 ? W - bw - 1 : Math.round((spineW - bw) / 2));
  ...
  // Opencode token set with legacy carthagent fallbacks (accent/ok/err/warn/muted/faint/hot).
  ...
  const boxes = [...info.entries()]
    .filter(([id]) => !(narrow && col.get(id) === 1))
    .map(([id, v]) => ({ id, top: v.top, bottom: v.bottom, h: v.h }));
  return { lines, hotRow, frontierId: frontier?.id || null, frontierLabel: frontier ? String(frontier.label) : '', boxes };
}
