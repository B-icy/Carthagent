/**
 * Best-effort clipboard *read* and *write* for the carthagent console.
 *
 ...
 * (pbcopy on macOS, wl-copy/xclip/xsel on Linux, PowerShell on Windows/WSL) so the
 * selected text lands in the OS pasteboard directly.
 *
 * carthagent stays dependency-free: these are all standard OS/desktop tools (or
 * PowerShell/clip on Windows/WSL), tried in order until one answers.
 */
import { execFile } from 'node:child_process';

const readWindows = () => ['powershell.exe', ['-NoProfile', '-Command', '[Console]::OutputEncoding = New-Object System.Text.UTF8Encoding($false); [Console]::Write((Get-Clipboard -Raw))']];
const writeWindows = () => [
  ['powershell.exe', ['-NoProfile', '-Command', '[Console]::InputEncoding = New-Object System.Text.UTF8Encoding($false); Set-Clipboard -Value ([Console]::In.ReadToEnd())']],
];
...
export function clipboardWriteCommands(platform = process.platform, env = process.env) {
  ...
  if (platform === 'win32') return writeWindows();
  if (platform !== 'linux') return [];
  const linux = [
    ['wl-copy', []],
    ...
    const next = () => {
      ...
      try {
        ...
        if (child.stdin) {
          ...
          child.stdin.end(Buffer.from(data, 'utf8'));
        }
      } catch {
        if (!handled) {
          handled = true;
