/** ANSI + text-measurement primitives for the carthagent TUI. Zero dependencies.
 *
 * Truecolor-forced like opencode: 24-bit SGR always, unless the terminal is
 * `dumb`, NO_COLOR is set, or CARTHAGENT_NO_TRUECOLOR=1 / CARTHAGENT_TRUECOLOR=0 opts out
...
export function detectColorDepth(env = process.env) {
  ...
  if (env.CARTHAGENT_NO_TRUECOLOR === '1' || env.CARTHAGENT_TRUECOLOR === '0') {
...
/** Slice a plain string to display columns [c0, c1); chars straddling the
 * boundary are included so copied text matches the visual selection. */
export function sliceCols(s, c0, c1) {
  let out = '', used = 0;
  for (const ch of String(s)) {
    const cw = charWidth(ch.codePointAt(0));
    if (used >= c1) break;
    if (used + cw > c0) out += ch;
    used += cw;
  }
  return out;
}

/** Wrap display columns [c0, c1) of a styled line in inverse video.
 * Lines shorter than c0 are padded with plain spaces up to c0 first, then
 * inverse padding up to c1, so multi-line selections read as a block. */
export function inverseCols(s, c0, c1) {
  const str = String(s);
  if (c1 <= c0) return str;
  const close = `${ESC}[27m`;
  let out = '', used = 0, i = 0, opened = false;
  while (i < str.length && used < c1) {
    if (str[i] === '\x1b') {
      const m = str.slice(i).match(/^\x1b\[[0-9;?]*[ -/]*[@-~]/);
      if (m) { out += m[0]; i += m[0].length; continue; }
    }
    ...
    const ch = String.fromCodePoint(cp);
    const cw = charWidth(cp);
    if (!opened && used + cw > c0) { out += INVERSE; opened = true; }
    out += ch;
    used += cw;
    i += ch.length;
  }
  ...
  out += str.slice(i);
  if (used < c1) {
    out += ' '.repeat(Math.max(0, c0 - used));
    out += INVERSE + ' '.repeat(c1 - Math.max(used, c0)) + close;
  }
  return out;
}
