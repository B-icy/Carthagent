#!/usr/bin/env node
/**
 * Mock the engine's `--mode rpc` for `carthagent demo`: speaks the rpc line protocol and plays a
...
const SPEED = Math.max(0.05, Number(process.env.CARTHAGENT_DEMO_SPEED) || 1);
...
let activeModel = { provider: 'demo', id: 'carthagent-demo-1' };
...
async function respond(cmd) {
  ...
  switch (cmd.type) {
    case 'get_state':
      return reply({
        model: activeModel, thinkingLevel,
    ...
    case 'cycle_model': return reply({ model: { provider: 'demo', id: 'carthagent-demo-1' } });
    case 'set_model': activeModel = { provider: cmd.provider, id: cmd.modelId }; return reply(activeModel);
    case 'get_available_thinking_levels': return reply({ levels: ['off', 'low', 'medium', 'high'] });
    case 'set_thinking_level': thinkingLevel = cmd.level; return reply({});
    case 'get_available_models': return reply({ models: [
      { provider: 'demo', id: 'carthagent-demo-1' },
      { provider: 'demo', id: 'carthagent-demo-2' },
      ...
      { provider: 'openrouter', id: 'deepseek/deepseek-v4.1-flash', name: 'DeepSeek: DeepSeek V4.1 Flash' },
    ] });
