/**
 * carthagent TUI — a split-terminal delivery console on top of `pi --mode rpc`.
 *
...
import { providersFromModels } from './models.mjs';
...
export async function runTui(opts) {
  ...
  // initial state + polls
  refreshState().then(s => {
    // `carthagent -c/--session` loaded history before the console attached — rebuild
  ...
  const choiceItems = (p) => {
    ...
    else if (p.row === 2) items = (state.modelList || []).filter(m => !p.provider || m.provider === p.provider).map(m => ({ ...m, value: m.full, label: `${m.provider} · ${m.id}` }));
    else items = (p.levels || []).map(value => ({ value, label: value }));
    const q = (p.search || '').toLowerCase();
    return items.filter(it => `${it.label} ${it.value}`.toLowerCase().includes(q));
  };
  ...
  const startReview = () => {
    ...
    rpc.send({ type: 'prompt', message: reviewKickoff(CARTHAGENT_BIN) }).catch(e => notice(feed, e.message, 'error'));
    state.scroll = 0;
  };
...
${BOLD}review${RESET} /review starts a PR ↔ fresh-review ↔ fixes loop · /review ask|yes|no sets the default · ^y accepts a pending offer`); break;
  ...
  function stampOverlay(lines, W, H, frame) {
    ...
    if (searching) {
      ...
      for (let i = 0; i < maxListRows; i++) {
        ...
        const name = truncate((m.value === current ? '[current] ' : '') + m.label, innerW - 4);
  ...
  const onKey = k => {
    ...
    if (state.overlay) {
      ...
      if (p.search !== undefined) {
        ...
        else if (k.key === 'char' || k.key === 'paste') { p.search += k.ch ?? k.text; p.sel = 0; p.error = ''; }
        else if (k.key === 'backspace') { p.search = p.search.slice(0, -1); p.sel = 0; p.error = ''; }
      } else {
        if (k.key === 'esc' || k.key === 'ctrl-t') state.overlay = null;
        else if (k.key === 'up') p.row = (p.row + 3) % 4;
        else if (k.key === 'down') p.row = (p.row + 1) % 4;
        else if (k.key === 'enter') { p.search = ''; p.sel = 0; p.error = ''; }
      }
      return;
    }
  ...
  function headerLines(W, frame) {
    ...
    const logo = fg(t.primary || t.accent) + BOLD + 'carthagent' + RESET;
