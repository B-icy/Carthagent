...
import { authTypeLabel, applyAuthEvent, createCloudBillingLink, filterLoginProviders, formatCloudAccount, formatCloudUsage, hasConfiguredProvider, loadAuthRuntime, loadCloudAccountUsage, loginAndRefresh, loginProviderFocus, loginProviderList, newAuthState, revokeCloudSession } from './auth.mjs';
import { cloudManagedUsageRecovery } from '../cloud/client.mjs';
...
import { modelDisplayIdentity, providerDisplayName } from '../providers/names.mjs';
...
const SLASH_COMMANDS = [
  ...
  { name: '/account', aliases: ['/cloud'], desc: 'show Ship usage, credit, plan, and sessions' },
  { name: '/usage', desc: 'show recent Ship usage' },
  { name: '/billing', args: '[checkout|portal]', desc: 'open Ship billing' },
  { name: '/logout-cloud', args: '[session-id]', desc: 'revoke a Ship CLI session' },
...
export function cloudManagedUsageNotice(event, activeModel = '') {
  const message = event?.message;
  if (event?.type !== 'message_end' || message?.role !== 'assistant' || message.stopReason !== 'error') return null;
  const provider = message.provider || String(activeModel).split('/')[0];
  return provider === 'experiential-labs' ? cloudManagedUsageRecovery(message.errorMessage) : null;
}

export function isCloudCreditExhaustion(event, activeModel = '') {
  return cloudManagedUsageNotice(event, activeModel) !== null;
}
...
export function syncActiveModel(state, feed, model) {
  ...
  for (const block of feed.blocks) {
    if (block.kind === 'welcome' && block.model !== modelDisplayIdentity(state.model)) {
      block.model = modelDisplayIdentity(state.model);
      changed = true;
    }
  }
...
export function computeNodeStates(delivery, checksRunning, checkRunId, currentHash) {
  ...
  else if (checksRunning) verify = 'active';
  else if (allFresh && status === 'verified') verify = 'done';
  else if (allFresh) verify = 'done';
  else if (status === 'verifying') verify = 'pending';
  ...
  states.set('review', status === 'verified' ? 'done' : (allFresh && !anyFail) || informative ? 'active' : 'pending');
...
export async function runTui(opts) {
  ...
  rpc.onEvent(ev => {
    ...
    applyEvent(feed, ev);
    const cloudRecovery = cloudManagedUsageNotice(ev, state.model);
    if (cloudRecovery && Date.now() - state.cloudRecoveryOfferedAt > 2_000) {
      state.cloudRecoveryOfferedAt = Date.now();
      notice(feed, cloudRecovery, 'warn');
    }
  ...
  const authFinish = (error) => {
    ...
    if (error) notice(feed, `login failed: ${error.message}`, 'error');
      ...
      for (const block of feed.blocks) if (block.kind === 'welcome') { block.noProvider = false; block.model = modelDisplayIdentity(state.model) || block.model; }
      feed.version++;
  ...
  const openAuth = async ({ focusProviderId } = {}) => {
    ...
    try {
      ...
      // When no provider is configured, always focus on Carthagent Ship (the recommended row).
  ...
  const usingCloudModel = () => String(state.model || '').split('/')[0] === 'experiential-labs';

  const requireCloudModel = command => {
    if (usingCloudModel()) return true;
    notice(feed, `${command} is available when using Carthagent Ship. Use /model to select a Ship model.`, 'warn');
    return false;
  };

  const showCloudAccount = async () => {
    if (!requireCloudModel('/account')) return;
    try {
      const { account, usage } = await loadCloudAccountUsage(await ensureAuthRuntime());
      push2(formatCloudAccount(account, { usage }));
      ...
      if (error.message === 'cloud_login_required') { notice(feed, 'Carthagent Ship sign-in required — opening /login', 'warn'); openAuth({ focusProviderId: 'experiential-labs' }); }
      else notice(feed, `Ship account: ${error.message}`, 'error');
    }
  };

  const showCloudUsage = async () => {
    if (!requireCloudModel('/usage')) return;
    try {
      const { account, usage } = await loadCloudAccountUsage(await ensureAuthRuntime());
      push2(formatCloudUsage(usage, { account }));
    } catch (error) {
      if (error.message === 'cloud_login_required') { notice(feed, 'Carthagent Ship sign-in required — opening /login', 'warn'); openAuth({ focusProviderId: 'experiential-labs' }); }
      else notice(feed, `Ship usage: ${error.message}`, 'error');
    }
  };
  ...
  const openCloudBilling = async action => {
    try {
      ...
      if (error.message === 'cloud_login_required') { notice(feed, 'Carthagent Ship sign-in required — opening /login', 'warn'); openAuth({ focusProviderId: 'experiential-labs' }); }
      else notice(feed, `Ship billing: ${error.message}`, 'error');
    }
  };
  ...
  const revokeCloud = async sessionId => {
    ...
    try {
      ...
      notice(feed, `revoked Ship session ${sessionId}`, 'info');
    } catch (error) { notice(feed, `Ship session: ${error.message}`, 'error'); }
  };
  ...
  const choiceItems = (p) => {
    ...
    else if (p.row === 1) items = [{ value: '', label: 'All providers' }, ...providersFromModels(state.modelList).map(value => ({ value, label: providerDisplayName(value) }))];
    else if (p.row === 2) items = (state.modelList || []).filter(m => !p.provider || m.provider === p.provider).map(m => ({ ...m, value: m.full, label: `${providerDisplayName(m.provider)} · ${m.name || m.id}`, description: m.name && m.name !== m.id ? m.id : '' }));
  ...
  const applyModel = async (m) => {
    ...
    notice(feed, `model → ${modelDisplayIdentity(state.model)}`, 'info');
  };
  ...
  const slash = v => {
    ...
    switch (cmd) {
      ...
      case '/usage': showCloudUsage(); break;
      case '/billing': {
...
${BOLD}cmds${RESET}  /login · /account · /usage · /billing [checkout|portal] · /logout-cloud <session> · /resume [id|name] · /restart · /theme [${THEME_NAMES.join('|')}] · /model [name] · /thinking · /compact · /new · /export [file] · /stats · /clear · /quit
${BOLD}review${RESET} /review starts a PR ↔ fresh-review ↔ fixes loop · /review ask|yes|no sets the default · ^y accepts a pending offer`); break;
  ...
  function stampOverlay(lines, W, H, frame) {
    ...
    const rows = searching ? [
      ...
      { label: 'current', value: state.overlay.row === 2 ? modelDisplayIdentity(state.model) : state.overlay.row === 0 ? currentThemeName() : state.overlay.row === 1 ? (providerDisplayName(state.overlay.provider) || 'All providers') : state.thinking },
    ] : [
      { label: 'theme', value: currentThemeName() },
      { label: 'provider', value: providerDisplayName(state.overlay.provider) || 'All providers' },
      { label: 'model', value: modelDisplayIdentity(state.model) || '—' },
    ...
    if (a.phase === 'providers' || a.phase === 'type') {
      ...
      for (let i = 0; i < maxRows; i++) {
        ...
        if (a.phase === 'type') {
          ...
          const labelColor = it.recommended ? (t.error || t.err || accent2) : (isSel ? accent : t.text);
          const label = fg(labelColor) + (it.recommended || isSel ? BOLD : '') + padEnd(truncate(it.name, room), room) + RESET;
          card.push(row(` ${marker} ${label} ${it.recommended ? fg(t.error || t.err || accent2) + recommendedBadge + RESET : ''} ${fg(faint)}${meta}${RESET}${it.configured ? fg(ok) + configuredBadge + RESET : ''}`));
        }
      }
  ...
  function headerLines(W, frame) {
    ...
    let model = modelDisplayIdentity(state.model);
  ...
  const welcome = { kind: 'welcome', model: modelDisplayIdentity(state.model || opts.model || ''), cwd, mode: opts.demo ? 'demo' : 'extension', noProvider, t: Date.now() };
  ...
  if (opts.openAuth) {
    ...
    // First run with no credentials: auto-open the provider picker focused on Carthagent Ship.
    await refreshState().catch(() => null);
