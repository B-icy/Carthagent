...
import { getTheme, resolveThemeName, THEME_NAMES } from './theme.mjs';
import { frameHint, framePrompt, shouldFrame } from './framing.mjs';
import { locatePi, buildPiArgs, packageInstalled, listSessions, mostRecentSession } from '../pi.mjs';
import { RESET, BOLD, ITALIC, fg, bg, width, strip, truncate, padEnd, SPINNER, bar, bgLine, colorDepth } from './ansi.mjs';

export { locatePi, buildPiArgs, packageInstalled };
...
class PiRpc {
  ...
  start({ cmd, args, cwd, env }) {
    ...
    for (const p of this.pending.values()) p.reject(new Error('pi process restarted'));
    this.pending.clear();
    ...
    this.proc.on('exit', (code, sig) => {
      ...
      for (const p of this.pending.values()) p.reject(new Error(`pi exited (${code})`));
      this.pending.clear();
    });
  ...
  send(cmd) {
    ...
    return new Promise((res, rej) => {
      if (this.exited || !this.proc?.stdin?.writable) return rej(new Error('pi process is not running'));
  ...
  kill() { try { this.proc?.kill('SIGTERM'); } catch { } setTimeout(() => { try { this.proc?.kill('SIGKILL'); } catch { } }, 800); }
}
...
class Screen {
  ...
  enter() {
    ...
    out.write('\x1b[?1049h\x1b[?25l\x1b[?1000h\x1b[?1006h\x1b[?2004h\x1b[2J');
  ...
  exit() {
    ...
    out.write('\x1b[?2004l\x1b[?1006l\x1b[?1000l\x1b[?25h\x1b[?2026l\x1b[?1049l');
  }
...
function makeKeyParser(onKey) {
  ...
  return chunk => {
    ...
    for (;;) {
      ...
      if (b === 64) onKey({ key: 'wheelup', ...at }); else if (b === 65) onKey({ key: 'wheeldown', ...at });
    }
    // escape sequences
    for (;;) {
      let done = false;
...
export async function runTui(opts) {
  ...
  const T = { theme: getTheme(resolveThemeName(opts.theme)) };
  ...
  const installed = !opts.demo && packageInstalled();
  const baseArgs = () => opts.demo ? piCmd.args : [...piCmd.args, '--mode', 'rpc', ...buildPiArgs({ ...opts, cwd }, { installed })];
  const spawnPi = (extraArgs = []) => rpc.start({ cmd: piCmd.cmd, args: [...baseArgs(), ...extraArgs], cwd });
  spawnPi();
  ...
  rpc.onEvent(ev => {
    if (ev.type === '__exit__') {
      notice(feed, `pi process exited (code ${ev.code}${ev.sig ? `, ${ev.sig}` : ''})${rpc.stderrTail ? ` — ${rpc.stderrTail.split('\n').filter(Boolean).pop()}` : ''} — /restart respawns`, 'error');
      state.running = false;
      state.abortAt = 0;
      return;
    }
    if (ev.type === '__error__') { notice(feed, `pi process error: ${ev.error.message}`, 'error'); return; }
    applyEvent(feed, ev);
    ...
    if (ev.type === 'agent_start') { state.running = true; state.abortAt = 0; state.runStarted = Date.now(); state.runEnded = 0; }
  ...
  /** Kill the pi child and respawn it on the same session file — the escape
  ...
  const restartPi = async (why = 'restart') => {
    ...
    notice(feed, `${why === 'force' ? 'pi unresponsive — force restarting' : 'restarting pi'}${resumeFile ? ' (resuming session)' : ''}…`, 'warn');
    ...
    try {
      ...
      notice(feed, `pi restarted · session ${String(state.sessionId || 'new').slice(0, 8)} · ${s.messageCount ?? 0} messages`, 'info');
      readHarness();
      feed.version++;
    } catch (e) {
      notice(feed, `pi restart failed: ${e.message}`, 'error');
    }
  };
  ...
  const abortRun = () => {
    ...
    notice(feed, 'aborting… esc/^c again force-restarts pi', 'warn');
  };
  ...
  const slash = v => {
    ...
    switch (cmd) {
      ...
      case '/help': push2(`${BOLD}keys${RESET}  enter send/steer · esc abort (again = force-restart pi) · ^r resume session · tab focus · ⇧tab view · pgup/pgdn/wheel scroll focused pane · ^t settings · ^n new session · ^x expand · ^l clear · ^c quit
${BOLD}input${RESET} plain text runs phased delivery (${state.autoFraming ? 'on' : 'off'}) · /raw <text> bypasses · /guide [task] toggles or runs · !<cmd> bash
${BOLD}cmds${RESET}  /resume [id|name] · /restart · /theme [${THEME_NAMES.join('|')}] · /model [name] · /thinking · /compact · /new · /export [file] · /stats · /clear · /quit`); break;
      case '/raw': {
        const v2 = arg.trim();
      ...
      case '/theme': {
        if (arg && THEME_NAMES.includes(arg.toLowerCase())) { T.theme = getTheme(arg.toLowerCase()); screen.prev = []; done(`theme → ${arg.toLowerCase()}`); }
  ...
  function currentThemeName() {
    return THEME_NAMES.find(k => getTheme(k) === T.theme) || resolveThemeName();
  }
  ...
  function cycleSetting(row, dir) {
    if (row === 0) {
      ...
      T.theme = getTheme(THEME_NAMES[(i + dir + THEME_NAMES.length) % THEME_NAMES.length]);
      screen.prev = []; // theme recolors every cell — force a full redraw
    } else if (row === 1) {
      rpc.send({ type: 'cycle_model' }).then(r => { if (r?.model) state.model = `${r.model.provider}/${r.model.id}`; }).catch(e => notice(feed, e.message, 'error'));
    } else if (row === 2) {
  ...
  const onKey = k => {
    ...
    switch (k.key) {
      case 'char': editor.insert(k.ch); break;
      case 'paste': editor.insert(k.text); break;
      ...
      case 'ctrl-u': editor.clear(); break;
      case 'ctrl-w': editor.wordBack(); break;
      case 'ctrl-k': editor.killEnd(); break;
  ...
  function frontierDetail(W, t, states, frontierId, frontierLabel) {
    ...
    const dot = st === 'done' ? fg(ok) + '✓' + RESET
      ...
      : fg(faint) + '○' + RESET;
    const head = `${dot} ${fg(muted)}${frontierId}${RESET} `;
  ...
  function render(frame) {
    ...
    screen.frame(lines, (state.overlay || state.sessions) ? null : { y: H - 2, x: box.cx });
  }
  ...
  const welcome = { kind: 'welcome', model: state.model || opts.model || '', cwd, mode: opts.demo ? 'demo' : (installed && !opts.isolate ? 'package' : 'extension'), t: Date.now() };
