...
import { authTypeLabel, applyAuthEvent, filterLoginProviders, loadAuthRuntime, loginAndRefresh, loginProviderFocus, loginProviderList, newAuthState } from './auth.mjs';
...
export async function runTui(opts) {
  ...
  const openAuth = async () => {
    ...
    try {
      ...
      a.sel = loginProviderFocus(a.providers, activeProviderId);
...
${BOLD}review${RESET} /review starts a PR ↔ fresh-review ↔ fixes loop · /review ask|yes|no sets the default · ^y accepts a pending offer`); break;
  ...
  function stampAuth(lines, W, H) {
    ...
    card.push(fg(bc) + '╭' + '─'.repeat(2) + RESET + fg(muted) + title + RESET + fg(bc) + '─'.repeat(Math.max(0, innerW - 2 - title.length)) + '╮' + RESET);
    if (a.phase === 'providers') card.push(row(` ${fg(faint)}Carthagent's managed gateway is recommended; direct providers remain available.${RESET}`));
    ...
    if (a.phase === 'providers' || a.phase === 'type') {
      ...
      for (let i = 0; i < maxRows; i++) {
        ...
        if (a.phase === 'type') {
          ...
          const room = Math.max(8, innerW - 4 - meta.length - badgeWidth);
