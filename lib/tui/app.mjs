...
${BOLD}cmds${RESET}  /resume [id|name] · /restart · /theme [${THEME_NAMES.join('|')}] · /model [name] · /thinking · /compact · /new · /export [file] · /stats · /clear · /quit`); break;
      ...
      case '/theme': {
        if (arg && THEME_NAMES.includes(arg.toLowerCase())) { T.theme = getTheme(arg.toLowerCase(), termMode); screen.prev = []; done(`theme → ${arg.toLowerCase()}`); }
  ...
  function cycleSetting(row, dir) {
    if (row === 0) {
      ...
      T.theme = getTheme(THEME_NAMES[(i + dir + THEME_NAMES.length) % THEME_NAMES.length], termMode);
