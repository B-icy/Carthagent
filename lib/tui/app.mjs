    ...
    if (a.phase === 'providers' || a.phase === 'type') {
      ...
      for (let i = 0; i < maxRows; i++) {
        ...
        if (a.phase === 'type') {
          ...
          const labelColor = it.recommended ? accent2 : (isSel ? accent : t.text);
          const label = fg(labelColor) + (it.recommended || isSel ? BOLD : '') + padEnd(truncate(it.name, room), room) + RESET;
          card.push(row(` ${marker} ${label} ${it.recommended ? fg(accent2) + recommendedBadge + RESET : ''} ${fg(faint)}${meta}${RESET}${it.configured ? fg(ok) + configuredBadge + RESET : ''}`));
        }
      }
