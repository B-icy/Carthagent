...
${BOLD}review${RESET} /review starts a PR ↔ fresh-review ↔ fixes loop · /review ask|yes|no sets the default · ^y accepts a pending offer`); break;
  ...
  function sideLines(W, H, frame) {
    ...
    // Always draw the full box graph — boxes, connectors and arrows. When the
    // plan outgrows the rail it scrolls (windowLines + the rail scrollbar)
    // rather than collapsing to a compact one-row-per-node list.
    const graph = full;
