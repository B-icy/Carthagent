...
import { planD2, computePhase, freshChecks, coverageReport, PHASES } from '../delivery.mjs';
import { parseD2, renderD2, renderD2Compact } from './d2.mjs';
...
import { authTypeLabel, applyAuthEvent, filterLoginProviders, loadAuthRuntime, loginProviderList, newAuthState, startLogin } from './auth.mjs';
import { readClipboard, sanitizePaste } from './clipboard.mjs';
import { cycleProvider, filterModelsByProvider, providerForModel, providersFromModels } from './models.mjs';
...
export async function runTui(opts) {
  ...
  let authAbort = null;
  let authPrompt = null; // { resolve, reject } for the pending pi prompt
  ...
  const authStart = (type) => {
    ...
    startLogin(authRuntime, a.provider.id, type, {
  ...
  const openAuth = async () => {
    ...
    if (state.auth) { state.auth = null; feed.version++; return; }
    ...
    try {
      ...
      a.providers = loginProviderList(authRuntime);
  ...
  const fetchModels = () => {
    ...
    modelsPromise = rpc.send({ type: 'get_available_models' }).then(r => {
      const list = (r?.models || []).map(m => ({ provider: m.provider, id: m.id, full: `${m.provider}/${m.id}` }));
  ...
  /** Provider currently shown in the settings overlay (falls back to the model's provider). */
  const overlayProvider = () => state.overlay?.provider || providerForModel(state.model, state.modelList) || providersFromModels(state.modelList)[0] || '';

  /** Filter the cached model list by the selected provider + search string. */
  const filterModels = q => filterModelsByProvider(state.modelList, state.overlay?.provider, q);

  /** Open the settings overlay on `row`, seeding the provider filter and priming models. */
  const openOverlay = (row) => {
    state.overlay = { row, provider: providerForModel(state.model, state.modelList) || null };
    fetchModels().then(list => {
      if (!state.overlay) return;
      if (!state.overlay.provider) state.overlay.provider = providerForModel(state.model, list) || providersFromModels(list)[0] || null;
      feed.version++;
    });
  };

  /** Change the overlay's provider filter, optionally switching the agent model
   *  to the first model of that provider so the header reflects the choice. */
  const setOverlayProvider = (provider, list) => {
    state.overlay.provider = provider;
    state.overlay.search = undefined;
    state.overlay.modelSel = 0;
    const mine = (list || state.modelList || []).filter(m => m.provider === provider);
    if (mine.length && !mine.some(m => m.full === state.model)) applyModel(mine[0]);
  };
...
${BOLD}review${RESET} /review starts a PR ↔ fresh-review ↔ fixes loop · /review ask|yes|no sets the default · ^y accepts a pending offer`); break;
  ...
  /** Cycle the overlay-selected setting: theme, provider, model, thinking. */
  function cycleSetting(row, dir) {
    if (row === 0) {
      ...
      fetchModels().then(list => {
        if (!state.overlay) return; // closed while models loaded
        const provs = providersFromModels(list);
        if (!provs.length) return;
        const next = cycleProvider(provs, overlayProvider(), dir);
        setOverlayProvider(next, list);
        feed.version++;
      });
    } else if (row === 2) {
      const list = filterModels('');
      if (!list.length) return;
      const i = list.findIndex(m => m.full === state.model);
      const at = i === -1 ? (dir > 0 ? -1 : 0) : i;
  ...
  function stampOverlay(lines, W, H, frame) {
    ...
    const rows = [
      ...
      { label: 'provider', value: overlayProvider() || '—', sub: !state.modelList ? 'loading models…' : (providersFromModels(state.modelList).length > 1 ? `${providersFromModels(state.modelList).length} providers · ←→ change` : '←→ change') },
   ...
   *  pi's login prompts inline — no terminal hand-off. */
  function stampAuth(lines, W, H) {
    ...
    if (a.phase === 'providers' || a.phase === 'type') {
      const items = a.phase === 'type'
        ? (a.provider?.types || []).map(ty => ({ id: ty, name: authTypeLabel(ty) }))
        : authVisible();
      ...
      for (let i = 0; i < maxRows; i++) {
        ...
        if (a.phase === 'type') {
          ...
          const meta = it.types.map(authTypeLabel).join(' · ');
          const badge = it.configured ? ` ${G.done} configured` : '';
          const room = Math.max(8, innerW - 4 - meta.length - badge.length);
          const label = (isSel ? fg(accent) + BOLD : fg(t.text)) + padEnd(truncate(it.name, room), room) + RESET;
          card.push(row(` ${marker} ${label} ${fg(faint)}${meta}${RESET}${it.configured ? fg(ok) + badge + RESET : ''}`));
        }
      }
  ...
  const onKey = k => {
    ...
    if (state.overlay) {
      ...
      if (state.overlay.search !== undefined) {
        switch (k.key) {
          ...
          case 'enter': {
            ...
            // Pick a model, then return to the settings popup (not all the way out).
            if (m) { applyModel(m); state.overlay.search = undefined; state.overlay.modelSel = 0; }
            break;
          }
          case 'char':
      ...
      switch (k.key) {
        ...
        case 'ctrl-c': cleanup(0); break;
        case 'up': state.overlay.row = (state.overlay.row + 3) % 4; break;
        case 'down': state.overlay.row = (state.overlay.row + 1) % 4; break;
        case 'left': cycleSetting(state.overlay.row, -1); break;
        case 'right': cycleSetting(state.overlay.row, 1); break;
        case 'enter': {
          if (state.overlay.row === 2) { state.overlay.search = ''; state.overlay.modelSel = 0; fetchModels(); }
          else cycleSetting(state.overlay.row, 1);
          break;
        }
        case 'char': {
          if (state.overlay.row === 2 && k.ch !== ' ') { state.overlay.search = k.ch; state.overlay.modelSel = 0; fetchModels(); }
  ...
  function headerLines(W, frame) {
    ...
    const spinChar = state.running ? fg(t.primary || t.accent) + spin(frame) + RESET : fg(t.textMuted || t.faint) + G.info + RESET;
    ...
    const left = pill ? `${logo}  ${spinChar} ${pill}` : `${logo}  ${spinChar}`;
  ...
  function deliveryPill(t) {
    ...
    if (st === 'verified') return fg(ok) + `${G.done} verified` + RESET;
    if (st === 'blocked') return fg(err) + `${G.blocked} blocked` + RESET;
    if (st) return fg(muted) + `${G.pending} ${st}` + RESET;
    return '';
  }
  ...
  function phaseLines(W, t) {
    ...
    const cells = order.map((p, i) => {
      const dot = i < index ? fg(ok) + G.phaseDone + RESET : i === index ? fg(accent) + G.phaseCurrent + RESET : fg(faint) + G.phasePending + RESET;
  ...
  function sideLines(W, H, frame) {
    ...
    let detail = frontierDetail(W, t, states, full.frontierId, full.frontierLabel);
    const graphH = H - out.length - checkRowsHeight(checkRows, state.delivery) - stats.length - cov.length - detail.length - 4;
    // The box graph costs 5 rows/rank (measured: 6 steps = 61 rows at width
    // 44), so a split-pane rail almost always overflows. Fall back to one row
    // per node when it doesn't fit so the whole plan stays visible.
    const graph = full.lines.length > Math.max(1, graphH)
      ? renderD2Compact(state.d2Graph, { width: W - 1, theme: t, states, glyphs: G })
      : full;
    const { lines, hotRow } = graph;
    detail = frontierDetail(W, t, states, graph.frontierId, graph.frontierLabel);
  ...
  function coverageLine(W, t) {
    ...
    const faint = t.textMuted || t.faint;
    const ok = t.success || t.ok, warn = t.warning || t.warn;
    const report = coverageReport(delivery.plan, delivery.evidence, state.currentHash);
  ...
  function windowLines(lines, h, hot, manual = PLAN_FOLLOW) {
    ...
    if (start > 0) win[0] = fg(faintMark) + G.fold + RESET;
    if (start + h < lines.length) win[win.length - 1] = fg(faintMark) + G.fold + RESET;
    win._start = start;
    return win;
  }
  ...
  function render(frame) {
    ...
    const planDot = state.view === 'split' ? (state.focusPane === 'plan' ? fg(accent0()) + G.phaseDone + RESET + ' ' : fg(faint) + G.phasePending + RESET + ' ') : '';
    ...
    if (showSide) {
      ...
      if (!planOnly) lines[2] = paintBase(padEnd(truncate(lines[2] || '', feedW), feedW) + sep + paintPanel(truncate(planTitleCore + planPosMark(), planContentW)));
