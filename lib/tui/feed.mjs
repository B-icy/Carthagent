 ...
 */
import { fg, BOLD, DIM, ITALIC, RESET, width, wrap, paint, mix, truncate, SPINNER } from './ansi.mjs';
 ...
 */
export function renderFeed(S, W, T, { now = Date.now(), expandResults = false, frame = 0 } = {}) {
  const lines = [];
  const blockRows = [];
  ...
  for (const b of S.blocks) {
    ...
    switch (b.kind) {
      ...
      case 'user': {
        body.push(`${fg(accent2)}${BOLD}▌ you${RESET}${b.via ? `  ${fg(faint)}⤷ ${b.via}${RESET}` : ''}`);
      ...
      case 'assistant': {
        ...
        for (const raw of b.text.split('\n')) {
          ...
          for (const l of wrap(styled, inner)) {
            if (first && !inFence.v) { body.push(`${fg(accent)}●${RESET} ${l}`); first = false; }
            else body.push(l);
          }
          if (first && inFence.v) first = false;
        }
        if (!b.text && b.streaming) { /* spinner below */ }
        if (b.streaming) body.push(`${fg(accent)}${SPINNER[frame % SPINNER.length]}${RESET}`);
        break;
      }
      case 'thinking': {
        body.push(`${fg(faint)}${ITALIC}· thinking${RESET}`);
        const t = tailLines(b.text, expandResults ? 40 : 4).join('\n');
        for (const l of wrap(t, inner)) body.push(`${fg(faint)}${ITALIC}${l}${RESET}`);
        if (b.streaming) body.push(`${fg(faint)}${ITALIC}${SPINNER[frame % SPINNER.length]}${RESET}`);
        break;
      }
      case 'tool': {
        const icon = TOOL_ICONS[b.name] || '•';
        const accentHot = T.accent || accent;
        const status = b.status === 'running' ? `${fg(accent)}${SPINNER[frame % SPINNER.length]}${RESET}`
          : b.status === 'done' ? `${fg(T.diffAdded || T.ok)}✓${RESET}`
          : b.status === 'error' ? `${fg(T.diffRemoved || T.err)}✗${RESET}`
          : `${fg(faint)}…${RESET}`;
        ...
        if (b.status !== 'running' && show.length) {
          ...
          for (const l of show) for (const w of wrap(l, inner - 2)) {
            const mark = firstTail ? fg(faint) + '⎿ ' + RESET : '  ';
      ...
      case 'bash': {
        ...
        if (b.status === 'running') body.push(`  ${fg(accent)}${SPINNER[frame % SPINNER.length]}${RESET}`);
      ...
      case 'notice': {
        ...
        const sym = b.tone === 'error' ? '✗' : b.tone === 'warn' ? '▲' : '·';
