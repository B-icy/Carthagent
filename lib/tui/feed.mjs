/**
 * Event → feed reducer for the carthagent TUI. Pure data: blocks describe what the
 * run terminal shows, derived stats describe the side panel. Rendering turns
 * blocks into styled lines (see renderFeed).
 */
import { fg, BOLD, DIM, ITALIC, RESET, wrap, truncate } from './ansi.mjs';
...
export function echoUser(S, text, queued, expanded) {
  // Phased delivery is the carthagent default — no badge on framed prompts.
 ...
 * Uses opencode markdown tokens with legacy carthagent fallbacks (accent2/accent). */
...
export function renderFeed(S, W, T, { now = Date.now(), expandResults = false, frame = 0, glyphs = UNICODE_GLYPHS } = {}) {
  ...
  for (const b of S.blocks) {
    ...
    switch (b.kind) {
      ...
      case 'welcome': {
        body.push(`${fg(accent)}${BOLD}carthagent${RESET}`);
        if (b.noProvider) {
          body.push(`${fg(T.warning || T.warn || accent2)}No AI provider connected.${RESET}`);
          body.push(`${fg(faint)}${truncate(b.cwd || '', inner)}${RESET}`);
          body.push('');
          body.push(`${fg(muted)}Connect a provider to get started.${RESET}`);
          body.push(`${fg(accent2)}Sign in to Carthagent Ship to start with trial credit.${RESET}`);
          body.push(`${fg(muted)}Press /login or Esc to dismiss this popup.${RESET}`);
        } else {
