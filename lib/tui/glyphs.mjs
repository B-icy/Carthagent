 ...
 *
 * carthagent draws a lot of icons — tool marks, status dots, a D2 graph and progress
 ...
 *
 * The tier is resolved once from `--ascii` / `CARTHAGENT_GLYPHS` / `CARTHAGENT_ASCII` and,
...
const U = {
  ...
  // pane scrollbars (box drawing — present in every monospace font)
  scrollTrack: '\u2502', // │
  scrollThumb: '\u2503', // ┃
  // phase strip dots
  phaseDone: '\u25cf',   // ●
...
export function detectGlyphMode(env = process.env, platform = process.platform) {
  const explicit = String(env.CARTHAGENT_GLYPHS || '').trim().toLowerCase();
  if (explicit === 'ascii' || explicit === 'unicode') return explicit;
  const flag = String(env.CARTHAGENT_ASCII || '').trim().toLowerCase();
  ...
  // Set CARTHAGENT_GLYPHS=unicode / --glyphs unicode to force the designed set.
...
export function resolveGlyphs({ mode, ascii, env = process.env, platform = process.platform } = {}) {
  // Only explicit ascii/unicode win; `auto`, undefined and unknown values
  // fall through to detection (so `--glyphs auto` behaves as documented).
  const explicit = mode === 'ascii' || mode === 'unicode' ? mode : ascii ? 'ascii' : null;
  const chosen = explicit || detectGlyphMode(env, platform);
...
/** True when a resolved set is the ASCII fallback tier. */
