...
export function formatCloudUsage(payload, { account } = {}) {
  const usage = Array.isArray(payload?.data) ? payload.data : [];
  const lines = [];
  if (account) {
    const subscription = account.subscription || {};
    lines.push(`Carthagent Ship · ${subscription.status && subscription.status !== 'none' ? `Builder (${subscription.status})` : 'Free'}`);
    lines.push(`Credit: ${formatNanoUsd(account.balance?.availableNanoUsd)} available`);
    const managed = account.managedUsage;
    if (managed) {
      lines.push(`Managed monthly: ${formatNanoUsd(managed.monthly?.availableNanoUsd)} available`);
      lines.push(`Managed daily: ${formatNanoUsd(managed.daily?.availableNanoUsd)} available${managed.daily?.resetsAt ? ` · resets ${managed.daily.resetsAt}` : ''}`);
    }
  }
  if (!usage.length) {
    lines.push('Ship usage: no settled requests yet');
    return lines.join('\n');
  }
  const knownInput = usage.filter(item => Number.isSafeInteger(item?.inputTokens));
  const knownOutput = usage.filter(item => Number.isSafeInteger(item?.outputTokens));
  const inputTokens = knownInput.reduce((sum, item) => sum + item.inputTokens, 0);
  const outputTokens = knownOutput.reduce((sum, item) => sum + item.outputTokens, 0);
  lines.push(`Ship usage (${usage.length} recent settled request${usage.length === 1 ? '' : 's'}): ${knownInput.length === usage.length ? formatTokenCount(inputTokens) : '?'} input · ${knownOutput.length === usage.length ? formatTokenCount(outputTokens) : '?'} output tokens`);
  for (const item of usage.slice(0, 5)) {
    const date = item?.createdAt ? new Date(item.createdAt) : null;
    const when = date && Number.isFinite(date.getTime()) ? date.toISOString() : 'unknown time';
    lines.push(`${when} · ${item?.alias || 'Ship model'} · ${formatTokenCount(item?.inputTokens)} in · ${formatTokenCount(item?.outputTokens)} out`);
  }
  return lines.join('\n');
}
...
export function formatCloudAccount(account, { usage } = {}) {
  ...
  const managed = account?.managedUsage;
  if (managed) {
    lines.push(`Managed monthly: ${formatNanoUsd(managed.monthly?.availableNanoUsd)} available of ${formatNanoUsd(managed.monthly?.allowanceNanoUsd)} (${formatNanoUsd(managed.monthly?.reservedNanoUsd)} reserved)`);
    lines.push(`Managed daily: ${formatNanoUsd(managed.daily?.availableNanoUsd)} available of ${formatNanoUsd(managed.daily?.limitNanoUsd)} (${formatNanoUsd(managed.daily?.reservedNanoUsd)} reserved)`);
    if (managed.daily?.resetsAt) lines.push(`Daily reset: ${managed.daily.resetsAt}`);
    const periodEnd = formatEpochSeconds(managed.periodEnd);
    if (periodEnd && periodEnd !== subscriptionEnd) lines.push(`Managed period ends: ${periodEnd}`);
