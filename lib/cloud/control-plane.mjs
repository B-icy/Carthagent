...
export class CloudControlPlane {
  ...
  issueSession(accountId, scopes = ['models:read', 'gateway:invoke', 'account:read']) {
    ...
    this.db.prepare(`INSERT INTO cli_sessions
      (id, account_id, refresh_token_hash, scopes, expires_at, created_at, last_used_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)`).run(sessionId, accountId, hash(refreshToken), scopes.join(' '), now + REFRESH_TTL_SECONDS, this.nowIso(), this.nowIso());
    return {
  ...
  pollDeviceToken({ deviceCode }) {
    ...
    const tokens = this.issueSession(row.account_id);
  ...
  revokeSession({ sessionId }) {
    this.db.prepare('UPDATE cli_sessions SET revoked_at = ? WHERE id = ? AND revoked_at IS NULL').run(this.nowIso(), sessionId);
    return { revoked: true };
  }

  listSessions(accountId) {
    return this.db.prepare('SELECT id, scopes, created_at, last_used_at, revoked_at FROM cli_sessions WHERE account_id = ? ORDER BY created_at DESC').all(accountId);
  }
