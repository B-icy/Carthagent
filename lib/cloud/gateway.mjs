...
export class ExperientialGatewayClient {
  ...
  async dispatch({ endpoint, body, operationKey, clientRequestId, signal }) {
    if (!this.apiKey) throw new GatewayDispatchError('Experiential gateway credential is not configured', { outcome: 'not_dispatched' });
    ...
    try {
      ...
      throw new GatewayDispatchError('Experiential gateway outcome is unknown', { outcome: 'unknown', cause });
    }
  }
}
...
export function pendingSettlementSource() {
  return {
    // Experiential's public OpenAI-compatible response contract currently
