...
export function cloudRouter(control, { authenticateBrowser, gateway = null } = {}) {
  ...
  const invokeGateway = endpoint => asyncRoute(async (req, res) => {
    ...
    try {
      ...
      if (error.message === 'insufficient_credit') return res.status(429).json({ error: { message: 'Carthagent Ship credit is exhausted. Manage billing or connect your own provider with /login.', type: 'insufficient_quota', code: 'insufficient_quota' } });
