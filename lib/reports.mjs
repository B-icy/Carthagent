...
import { activeReportPath } from './workspace.mjs';

export function latestReport(cwd) {
  const selected = activeReportPath(cwd);
  if (selected) return { path: selected, dir: dirname(selected), mtimeMs: statSync(selected).mtimeMs, state: JSON.parse(readFileSync(selected, 'utf8')) };
