...
const OMIT = new Set(['.git', '.venv', 'venv', 'node_modules', '__pycache__', '.harness', 'artifacts', 'saves', '.tools', '.pytest_cache', '.ruff_cache', 'dist', 'build', '.next', '.nuxt', '.cache', 'coverage', '.turbo', 'target', '.pi', '.carthagent', '.yarn', '.pnpm-store', '.output', '.svelte-kit', '.docusaurus']);
const DEFAULT_MAX_FILES = 25000;
const DEFAULT_MAX_BYTES = 256 * 1024 * 1024;

export function maxEvidenceFiles() {
  const envVal = process.env.CARTHAGENT_MAX_FILES || process.env.PI_MAX_FILES;
  if (envVal) {
...
export function maxEvidenceBytes() {
  const envVal = process.env.CARTHAGENT_MAX_BYTES || process.env.PI_MAX_BYTES;
  if (envVal) {
    const parsed = parseInt(envVal, 10);
    if (!Number.isNaN(parsed) && parsed > 0) return parsed;
  }
 ...
 * - check tool currently running -> verify (index 3)
 * - all checks fresh   -> review (index 4)
 *
 * `status="verifying"` is persisted while a check runs, but it is not proof
 * that a check is still active. A process can stop between report writes, so
 * presentation must prefer current evidence and the live `checkRunning` signal.
...
export function computePhase(delivery, hash, checkRunning = false) {
  ...
  if (checkRunning) return { phase: 'verify', index: 3, revision };
