...
export function evidenceFresh(check, evidence, hash) {
  return Boolean(evidence?.passed && evidence.fingerprint === hash &&
    evidence.checkDigest === checkDigest(check));
}

export function pendingChecks(state, hash) {
  return state.plan.checks.filter(c => !evidenceFresh(c, state.evidence?.[c.id], hash)).map(c => c.id);
}
 ...
 */
export function coverageReport(plan, evidence = {}, hash = '') {
  ...
  return (plan?.acceptance || []).map(item => {
    ...
    for (const id of item.checks || []) {
      ...
      if (!c || !evidenceFresh(c, e, hash)) { stale++; continue; }
      fresh++;
...
export function freshChecks(state, hash) {
  ...
  for (const c of state.plan.checks) {
    ...
    if (evidenceFresh(c, e, hash)) fresh++;
  }
