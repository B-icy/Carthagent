...
const OMIT = new Set(['.git', '.venv', 'venv', 'node_modules', '__pycache__', '.harness', 'artifacts', 'saves', '.tools', '.pytest_cache', '.ruff_cache', 'dist', 'build', '.next', '.nuxt', '.cache', 'coverage', '.turbo', 'target']);
...
export function coverageReport(plan, evidence = {}, hash = '') {
  ...
  return (plan?.acceptance || []).map(item => {
    ...
    for (const id of item.checks || []) {
      ...
      if (hash && e.fingerprint && e.fingerprint !== hash) { stale++; continue; }
      fresh++;
...
export function freshChecks(state, hash) {
  ...
  for (const c of state.plan.checks) {
    ...
    if (e?.passed && e.fingerprint === hash) fresh++;
  }
