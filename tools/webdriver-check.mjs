...
try {
  ...
  for (let i = 0; i < args.length; i += 2) {
    ...
    if (!['--endpoint', '--root', '--page', '--assert', '--assert-text', '--click', '--screenshot'].includes(key) || value === undefined || value.startsWith('--')) throw Error(`Unsupported or missing option: ${key}`);
  ...
  const url = new URL(options.page || 'index.html', server.url + '/').href;
  if (new URL(url).origin !== server.url) throw Error('--page must address the local served workspace');
  const result = await webdriverCheck({ endpoint: options.endpoint, url, steps, screenshot: options.screenshot && resolve(options.screenshot) });
