...
Keys: `enter` send (or steer mid-run) · `esc` abort/clear — press again while still running to force-restart a wedged pi process (the session file is resumed, nothing is lost) · `^r` session picker · `tab` focus feed/plan pane · `⇧tab` cycle split/feed/plan views · `↑↓` history or scroll focused pane · `pgup/pgdn`/wheel scroll focused pane · drag-select copies to the clipboard · `x` expand tool output · `^t` settings overlay (theme/model/thinking) · `^n` new session · `^l` clear feed · `^c` quit (aborts a running agent first, then force-restarts).
